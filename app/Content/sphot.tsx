...
function SpHotShowHome() {
    const [arrSpHots, setArrSpHots] = useState<OneProd[]>([]);
    const [arrBrands, setArrBrands] = useState<OneBrand[]>([]); 
    const [isLoading, setIsLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
  
    useEffect(() => {
      const fetchProducts = fetch(`http://localhost:3000/api/sp_danh_gia_cao`).then(res => {
        if (!res.ok) {
      ...
      <section className="w-6/7 m-auto">
        <div className="container px-6 py-10 mx-auto">
          <div className="flex flex-col items-center animate-pulse">
            ...
            <p className="mt-6 text-xl">Các sản phẩm được đánh giá cao</p>
            <h1 className="text-3xl font-medium">
              Khám phá các sản phẩm được đánh giá cao nhất của chúng tôi
            </h1>
          </div>
  
          <div className="grid grid-cols-1 gap-8 mt-8 xl:mt-12 xl:gap-12 sm:grid-cols-2 xl:grid-cols-4 lg:grid-cols-3">
            {arrSpHots.map((sphot: OneProd) => (
              <ShowSP
                key={sphot._id}
                sp={{ ...sphot, ten_brand: getBrandName(sphot.id_brand) }} // Thêm ten_brand vào sp
              />
            ))}
          </div>
        </div>
